...
def handle_class(name, member):
    ...
    if member.__doc__ is None and not member_too_small(member):
        raise ValueError(
            f"{name} class doesn't have any documentation",
            member.__module__,
...
def handle_function(name, member):
    ...
    if doc is None:
        raise ValueError(
            f"{name} function doesn't have any documentation",
            member.__module__,
            inspect.getmodule(member).__file__,
        )
...
def assert_doc_style(name, member, doc):
    ...
    if len(first_line.strip()) == 0:
        raise ValueError(
            f"{name} the documentation should be on the first line.",
            member.__module__,
        )

    first_blank = [i for i, line in enumerate(lines) if not line.strip()]
    if first_blank and lines[first_blank[0] - 1].strip()[-1] != '.':
        raise ValueError(f"{name} first line should end with a '.'", member.__module__)
...
def assert_function_style(name, member, doc, args):
    ...
    if has_return and "# Returns" not in doc:
        ...
        if len(return_in_sub) < len(has_return):
            raise ValueError(f"{name} needs a '# Returns' section", member.__module__)
    ...
    if has_raise and "# Raises" not in doc:
        ...
        if len(raise_in_sub) < len(has_raise):
            raise ValueError(f"{name} needs a '# Raises' section", member.__module__)

    if len(args) > 0 and "# Arguments" not in doc:
        raise ValueError(f"{name} needs a '# Arguments' section", member.__module__)
    ...
    for keyword in keywords:
        if keyword in doc_lines:
            ...
            if doc_lines[index - 1] != '':
                raise ValueError(
                    f"{name} '{keyword}' should have a blank line above.",
                    member.__module__,
                )
...
def assert_args_presence(args, doc, member, name):
    ...
    styles = [f"{arg}:" not in words for arg in args]
    if any(styles):
        raise ValueError(
            f"{name} {list(compress(args, styles))} are not style properly 'argument': documentation",
            member.__module__,
        )


    # Check arguments order
    indexes = [words.index(f"{arg}:") for arg in args]
    if indexes != sorted(indexes):
        raise ValueError(
            f"{name} arguments order is different from the documentation",
            member.__module__,
            indexes,
        )
